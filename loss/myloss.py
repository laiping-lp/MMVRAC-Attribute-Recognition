...
class Pedal(nn.Module):
    ...
    def forward(self, feature, centers, position, PatchMemory = None, vid=None, camid=None):
        ...
        for p in range(feature.size(0)):
            ...
            # neg = F.normalize(neg, dim=1) ########## add by me
            # neg = neg / neg.max().detach() * 30 ####### add by me
            x = ((-1 * self.scale * neg[:, :self.k]).exp().sum(dim=1)).log()
            y = ((-1 * self.scale * neg).exp().sum(dim=1)).log()

            # x = ((-1 * neg[:, :self.k]).exp().sum(dim=1)).log() ####
            # y = ((-1 * neg).exp().sum(dim=1)).log() #####
            ...
            if torch.isnan(l).item():
                ...
                # import ipdb
                # ipdb.set_trace()
            if torch.isinf(l).item():
                print("="*20+" inf "+"="*20)
                # import ipdb
                # ipdb.set_trace()
            l = torch.where(torch.isnan(l), torch.full_like(l, 0.), l) # add by me
            loss += l
